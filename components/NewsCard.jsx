...
const NewsCard = async ({ dictionary }) => {
              ...
              <article className="group h-full rounded-2xl border border-white/10 bg-[var(--surface)] p-5 shadow-[0_24px_50px_rgba(5,8,16,0.55)] transition hover:-translate-y-1 hover:border-cyan-300/40">
                ...
                <div className="mt-4 flex items-center justify-between text-xs text-zinc-400">
                  <div className="flex items-center gap-2">
                    <Image
                      src={`/${articale.author_avatar}`}
                      alt="Author"
                      className="h-7 w-7 rounded-full object-cover"
                      height={28}
                      width={28}
                    />
                    <span>{articale.author_name}</span>
                  </div>
                  <span>1 day ago</span>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between text-xs text-zinc-400">
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1">
                      ...
                      {formatCompactNumber(articale.upvotes)}
                    </span>
                    <span className="flex items-center gap-1">
                      <svg
                        viewBox="0 0 24 24"
                      ...
                      {formatCompactNumber(articale.downvotes)}
                    </span>
                  </div>
                  <span className="flex items-center gap-1">
                    <svg
                    ...
                    {formatCompactNumber(articale.views)}
                  </span>
                </div>
              </article>
            </Link>
